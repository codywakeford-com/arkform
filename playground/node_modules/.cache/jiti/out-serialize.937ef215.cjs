"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.snapshot = exports.printable = exports.print = exports.describeCollapsibleDate = void 0;var _domain = require("./domain.js");
...
const print = (data, opts) => console.log(printable(data, opts));exports.print = print;
const printable = (data, opts) => {
  switch ((0, _domain.domainOf)(data)) {
    case "object":
      const o = data;
    ...
    default:
      return (0, _primitive.serializePrimitive)(data);
  }
...
const stringifyUnquoted = (value, indent, currentIndent) => {
  ...
  return (0, _primitive.serializePrimitive)(value);
  ...
  if (ctorName === "Object") {
    const keyValues = (0, _records.stringAndSymbolicEntriesOf)(value).map(([key, val]) => {
      const stringifiedKey = typeof key === "symbol" ? printableOpts.onSymbol(key) :
      (0, _registry.isDotAccessible)(key) ? key :
      JSON.stringify(key);
...
const printableOpts = {
  onCycle: () => "(cycle)",
  onSymbol: (v) => `Symbol(${(0, _registry.register)(v)})`,
  onFunction: (v) => `Function(${(0, _registry.register)(v)})`
};
...
const describeCollapsibleDate = (date) => {
  ...
  if (milliseconds)
  timePortion += `.${pad(milliseconds, 3)}`;else
  if (timeWithUnnecessarySeconds.test(timePortion))
  timePortion = timePortion.slice(0, -3);
  return `${timePortion + suffix}, ${datePortion}`;
};exports.describeCollapsibleDate = describeCollapsibleDate;
const months = [
"January",
"February",
"March",
...
const pad = (value, length) => String(value).padStart(length, "0"); /* v9-9ea36e7757f26cbc */
