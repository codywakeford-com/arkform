"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.postfixWithoutVariadicMessage = exports.postfixAfterOptionalOrDefaultableMessage = exports.SequenceNode = exports.Sequence = void 0;var _util = require("@ark/util");
...
const implementation = (0, _implement.implementNode)({
    ...
    defaultables: {
      ...
      parse: (defaultables, ctx) => {
        ...
        return defaultables.map((element) => {
          ...
          (0, _optional.assertDefaultValueAssignability)(node, element[1], null);
          return [node, element[1]];
        });
      },
      serialize: (defaults) => defaults.map((element) => [
      element[0].collapsibleJson,
      (0, _implement.defaultValueSerializer)(element[1])]
      )
    },
    variadic: {
      child: true,
  ...
  normalize: (schema) => {
    ...
    "minVariadicLength" in schema) {
      if (schema.postfix?.length) {
        if (!schema.variadic)
        return (0, _util.throwParseError)(postfixWithoutVariadicMessage);
        if (schema.optionals?.length || schema.defaultables?.length)
        return (0, _util.throwParseError)(postfixAfterOptionalOrDefaultableMessage);
      }
      if (schema.minVariadicLength && !schema.variadic) {
        return (0, _util.throwParseError)("minVariadicLength may not be specified without a variadic element");
      }
      return schema;
    }
    return { variadic: schema };
  ...
  defaults: {
    description: (node) => {
      ...
      `${element.node.nestableExpression} = ${(0, _util.printable)(element.default)}` :
  ...
  intersections: {
    sequence: (l, r, ctx) => {
      const rootState = _intersectSequences({
        l: l.tuple,
        r: r.tuple,
        disjoint: new _disjoint.Disjoint(),
        result: [],
        fixedVariants: [],
        ctx
      });
...
class SequenceNode extends _constraint.BaseConstraint {
  impliedBasis = _registry.$ark.intrinsic.Array.internal;
  ...
  variadicOrPostfix = (0, _util.conflatenate)(this.variadic && [this.variadic], this.postfix);
  // have to wait until prevariadic and variadicOrPostfix are set to calculate
  flatRefs = this.addFlatRefs();
  addFlatRefs() {
    (0, _node.appendUniqueFlatRefs)(this.flatRefs, this.prevariadic.flatMap((element, i) => (0, _util.append)(element.node.flatRefs.map((ref) => (0, _node.flatRef)([`${i}`, ...ref.path], ref.node)), (0, _node.flatRef)([`${i}`], element.node))));
    (0, _node.appendUniqueFlatRefs)(this.flatRefs, this.variadicOrPostfix.flatMap((element) =>
    // a postfix index can't be directly represented as a type
    // key, so we just use the same matcher for variadic
    (0, _util.append)(element.flatRefs.map((ref) => (0, _node.flatRef)([_registry.$ark.intrinsic.nonNegativeIntegerString.internal, ...ref.path], ref.node)), (0, _node.flatRef)([_registry.$ark.intrinsic.nonNegativeIntegerString.internal], element))));
  ...
  (0, _registry.registeredReference)(this.defaultValueMorphs) :
  undefined;
  elementAtIndex(data, index) {
    ...
    return {
      kind: "variadic",
      node: this.variadic ??
      (0, _util.throwInternalError)(`Unexpected attempt to access index ${index} on ${this}`)
    };
  }
  ...
  traverseApply = (data, ctx) => {
    let i = 0;
    for (; i < data.length; i++) {
      (0, _traversal.traverseKey)(i, () => this.elementAtIndex(data, i).node.traverseApply(data[i], ctx), ctx);
    }
  };
  get element() {
  ...
  _transform(mapper, ctx) {
    ctx.path.push(_registry.$ark.intrinsic.nonNegativeIntegerString.internal);
...
const getDefaultableMorphs = (node) => {
  ...
  for (let i = node.prefixLength; i <= lastDefaultableIndex; i++) {
    ...
    morphs.push((0, _optional.computeDefaultValueMorph)(i, elementNode, defaultValue));
    cacheKey += `${i}: ${elementNode.id} = ${(0, _implement.defaultValueSerializer)(defaultValue)}, `;
  }
  cacheKey += "]";
  return defaultableMorphsCache[cacheKey] ??= morphs;
};
const Sequence = exports.Sequence = {
  implementation,
  Node: SequenceNode
};
...
const sequenceTupleToInner = (tuple) => tuple.reduce((result, element) => {
  ...
  result.variadic = element.node;else
  if (element.kind === "defaultables") {
    result.defaultables = (0, _util.append)(result.defaultables, [
    [element.node, element.default]]
    );
  } else

  result[element.kind] = (0, _util.append)(result[element.kind], element.node);
  return result;
}, {});
const postfixAfterOptionalOrDefaultableMessage = exports.postfixAfterOptionalOrDefaultableMessage = "A postfix required element cannot follow an optional or defaultable element";
const postfixWithoutVariadicMessage = exports.postfixWithoutVariadicMessage = "A postfix element requires a variadic element";
const _intersectSequences = (s) => {
  ...
  const result = (0, _intersections.intersectOrPipeNodes)(lHead.node, rHead.node, s.ctx);
  if (result instanceof _disjoint.Disjoint) {
    if (kind === "prefix" || kind === "postfix") {
      ...
      s.result = [...s.result, { kind, node: _registry.$ark.intrinsic.never.internal }];
    } else
    if (kind === "optionals" || kind === "defaultables") {
  ...
  if (kind === "defaultables") {
    if (lHead.kind === "defaultables" &&
    rHead.kind === "defaultables" &&
    lHead.default !== rHead.default) {
      (0, _util.throwParseError)((0, _prop.writeDefaultIntersectionMessage)(lHead.default, rHead.default));
    }
    s.result = [
    ...s.result,
    {
      ...
      (0, _util.throwInternalError)(`Unexpected defaultable intersection from ${lHead.kind} and ${rHead.kind} elements.`)
    }];
