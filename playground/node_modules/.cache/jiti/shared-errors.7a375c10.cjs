"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.TraversalError = exports.ArkErrors = exports.ArkError = void 0;var _util = require("@ark/util");
var _utils = require("./utils.js");
class ArkError extends _util.CastableBase {
  [_utils.arkKind] = "error";
  path;
  data;
  nodeConfig;
  input;
  ...
  constructor({ prefixPath, relativePath, ...input }, ctx) {
    ...
    this.input = input;
    this.ctx = ctx;
    (0, _util.defineProperties)(this, input);
    const data = ctx.data;
    if (input.code === "union") {
      input.errors = input.errors.flatMap((innerError) => {
        ...
        return flat.map((e) => e.transform((e) => ({
          ...e,
          path: (0, _util.conflatenateAll)(prefixPath, e.path, relativePath)
        })));
      });
    }
    ...
    this.path = new _util.ReadonlyPath(...basePath);
  ...
  get propString() {
    return (0, _util.stringifyPath)(this.path);
  }
  get expected() {
...
class ArkErrors extends _util.ReadonlyArray {
  [_utils.arkKind] = "errors";
  ctx;
  constructor(ctx) {
    super();
    this.ctx = ctx;
   ...
   */
  get flatByPath() {
    return (0, _util.flatMorph)(this.byPath, (k, v) => [k, v.flat]);
  }
  /**
   * {@link byPath} flattened so that each value is an array of problem strings at that path.
   */
  get flatProblemsByPath() {
    return (0, _util.flatMorph)(this.byPath, (k, v) => [k, v.flat.map((e) => e.problem)]);
  }
  /**
  ...
  addAncestorPaths(error) {
    for (const propString of error.path.stringifyAncestors()) {
      this.byAncestorPath[propString] = (0, _util.append)(this.byAncestorPath[propString], error);
    }
  }
}exports.ArkErrors = ArkErrors;
class TraversalError extends Error {
  name = "TraversalError";
  constructor(errors) {
    if (errors.length === 1)
    super(errors.summary);else
