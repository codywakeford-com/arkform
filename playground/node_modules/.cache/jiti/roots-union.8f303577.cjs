"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.writeOrderedIntersectionMessage = exports.writeIndiscriminableMorphMessage = exports.reduceBranches = exports.pruneDiscriminant = exports.intersectBranches = exports.describeBranches = exports.UnionNode = exports.Union = void 0;var _util = require("@ark/util");
...
const implementation = (0, _implement.implementNode)({
    ...
    branches: {
      ...
      parse: (schema, ctx) => {
        ...
        for (const branchSchema of schema) {
          const branchNodes = (0, _utils.hasArkKind)(branchSchema, "root") ?
  ...
  normalize: (schema) => (0, _util.isArray)(schema) ? { branches: schema } : schema,
  reduce: (inner, $) => {
  ...
  defaults: {
    ...
    expected: (ctx) => {
      ...
      const pathDescriptions = Object.entries(byPath).map(([path, errors]) => {
        ...
        errors[0].actual :
        (0, _util.printable)(errors[0].data);
  ...
  intersections: {
    union: (l, r, ctx) => {
      if (l.isNever !== r.isNever) {
        ...
        return _disjoint.Disjoint.init("presence", l, r);
      }
      let resultBranches;
      if (l.ordered) {
        if (r.ordered) {
          (0, _util.throwParseError)(writeOrderedIntersectionMessage(l.expression, r.expression));
        }
        resultBranches = intersectBranches(r.branches, l.branches, ctx);
        if (resultBranches instanceof _disjoint.Disjoint)
    ...
    ...(0, _utils2.defineRightwardIntersections)("union", (l, r, ctx) => {
      const branches = intersectBranches(l.branches, [r], ctx);
      if (branches instanceof _disjoint.Disjoint)
      return branches;
...
class UnionNode extends _root.BaseRoot {
  ...
  get branchGroups() {
    ...
    for (const branch of this.branches) {
      if (branch.hasKind("unit") && branch.domain === "boolean") {
        ...
        branchGroups[firstBooleanIndex] = _registry.$ark.intrinsic.boolean;
        continue;
      }
      branchGroups.push(branch);
    }
  ...
  createBranchedOptimisticRootApply() {
    return (data, onFail) => {
      ...
      if (optimisticResult !== _util.unset)
  ...
  get shallowMorphs() {
    return this.branches.reduce((morphs, branch) => (0, _util.appendUnique)(morphs, branch.shallowMorphs), []);
  }
  ...
  innerToJsonSchema(ctx) {
    ...
    this.branchGroups[0].equals(_registry.$ark.intrinsic.boolean))
    return { type: "boolean" };
    ...
    Object.keys(branch).length === 1 && (0, _util.hasKey)(branch, "const"))) {
      return {
  ...
  compile(js) {
    ...
    js.block(`switch(${condition})`, () => {
      for (const k in cases) {
        ...
        v :
        optimistic ?
        `${js.invoke(v)} ? ${v.contextFreeMorph ? `${(0, _registry.registeredReference)(v.contextFreeMorph)}(${js.data})` : js.data} : "${_util.unset}"` :
        js.invoke(v)}`);
      }
      return js;
    });
    if (js.traversalKind === "Allows") {
      js.return(optimistic ? `"${_util.unset}"` : false);
      return;
    }
    ...
    caseKeys.map((k) => {
      ...
      _util.domainDescriptions[jsTypeOf];
    }) :
    caseKeys);
    const serializedPathSegments = this.discriminant.path.map((k) => typeof k === "symbol" ? (0, _registry.registeredReference)(k) : JSON.stringify(k));
  ...
  compileIndiscriminable(js) {
    ...
    } else
    {
      ...
      for (const branch of this.branches) {
        ...
        `${(0, _registry.registeredReference)(branch.contextFreeMorph)}(${js.data})` :
        js.data :
        true));
      }
      js.return(optimistic ? `"${_util.unset}"` : false);
  ...
  discriminate() {
    ...
    if (this.unitBranches.length === this.branches.length) {
      const cases = (0, _util.flatMorph)(this.unitBranches, (i, n) => [
    ...
    for (let lIndex = 0; lIndex < this.branches.length - 1; lIndex++) {
      ...
      for (let rIndex = lIndex + 1; rIndex < this.branches.length; rIndex++) {
        ...
        const result = (0, _intersections.intersectNodesRoot)(l.in, r.in, l.$);
        ...
        for (const entry of result) {
          ...
          continue;
          const matching = candidates.find((d) => (0, _util.arrayEquals)(d.path, entry.path) && d.kind === entry.kind);
          if (!matching) {
            candidates.push({
              kind: entry.kind,
              cases: {
          ...
          } else
          {
            if (matching.cases[lSerialized]) {
              matching.cases[lSerialized].branchIndices = (0, _util.appendUnique)(matching.cases[lSerialized].branchIndices, lIndex);
            } else
            {
            ...
            if (matching.cases[rSerialized]) {
              matching.cases[rSerialized].branchIndices = (0, _util.appendUnique)(matching.cases[rSerialized].branchIndices, rIndex);
            } else
            {
...
const resolveCase = (ctx, key) => {
  ...
  for (let i = 0; i < ctx.defaultEntries.length; i++) {
    ...
    resolvedEntries?.push(entry);else
    {
...
const discriminantCaseToNode = (caseDiscriminant, path, $) => {
  ...
  for (let i = path.length - 1; i >= 0; i--) {
    ...
    node = $.node("intersection", typeof key === "number" ?
    {
      ...
      sequence: [...(0, _util.range)(key).map((_) => ({})), node]
    } :
    {
      domain: "object",
...
const optionallyChainPropString = (path) => path.reduce((acc, k) => acc + (0, _compile.compileLiteralPropAccess)(k, true), "data");
const serializedTypeOfDescriptions = (0, _registry.registeredReference)(_util.jsTypeOfDescriptions);
const serializedPrintable = (0, _registry.registeredReference)(_util.printable);
...
const discriminantToJson = (discriminant) => ({
  ...
  path: discriminant.path.map((k) => typeof k === "string" ? k : (0, _compile.compileSerializedValue)(k)),
  cases: (0, _util.flatMorph)(discriminant.cases, (k, node) => [
  k,
  node === true ? node :
...
};exports.describeBranches = describeBranches;
const intersectBranches = (l, r, ctx) => {
  ...
  for (let lIndex = 0; lIndex < l.length; lIndex++) {
    ...
    for (let rIndex = 0; rIndex < r.length; rIndex++) {
      ...
      const branchIntersection = (0, _intersections.intersectOrPipeNodes)(l[lIndex], r[rIndex], ctx);
      if (branchIntersection instanceof _disjoint.Disjoint) {
  ...
  _disjoint.Disjoint.init("union", l, r) :
  resultBranches;
};exports.intersectBranches = intersectBranches;
const reduceBranches = ({ branches, ordered }) => {
  ...
  for (let i = 0; i < branches.length; i++) {
    for (let j = i + 1; j < branches.length && uniquenessByIndex[i] && uniquenessByIndex[j]; j++) {
      ...
      const intersection = (0, _intersections.intersectNodesRoot)(branches[i].in, branches[j].in, branches[0].$);
      ...
      if (intersection.equals(branches[j].in))
      uniquenessByIndex[j] = false;
    }
  }
  return branches.filter((_, i) => uniquenessByIndex[i]);
};exports.reduceBranches = reduceBranches;
const assertDeterminateOverlap = (l, r) => {
  if (!l.includesTransform && !r.includesTransform)
  return;
  if (!(0, _util.arrayEquals)(l.shallowMorphs, r.shallowMorphs)) {
    (0, _util.throwParseError)(writeIndiscriminableMorphMessage(l.expression, r.expression));
  }
  if (!(0, _util.arrayEquals)(l.flatMorphs, r.flatMorphs, {
...
});exports.pruneDiscriminant = pruneDiscriminant;
...
Left: ${lDescription}
Right: ${rDescription}`;exports.writeIndiscriminableMorphMessage = writeIndiscriminableMorphMessage;
const writeOrderedIntersectionMessage = (lDescription, rDescription) => `The intersection of two ordered unions is indeterminate:
Left: ${lDescription}
Right: ${rDescription}`;exports.writeOrderedIntersectionMessage = writeOrderedIntersectionMessage; /* v9-d80bd4c7f0d442b2 */
